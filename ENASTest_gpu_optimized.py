...
if tf.config.list_physical_devices('GPU'):
    print("✅ GPU detected. Enabling mixed precision for faster training.")
...
val_ds = tf.data.Dataset.from_tensor_slices((x_train, y_train)).take(10000).batch(32).cache().prefetch(tf.data.AUTOTUNE)
...
def build_model(hp):
    ...
    model.add(layers.Dense(10, activation='softmax', dtype='float32'))  # Ensure softmax is float32
...
history = best_model.fit(train_ds, epochs=10, validation_data=val_ds)
...
for img, label in test_ds.unbatch().take(num_samples):
